...
async def send_digest(context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[
        InlineKeyboardButton('👍', callback_data='like:' + str(article_ids[0])),
        InlineKeyboardButton('👎', callback_data='dislike:' + str(article_ids[0])),
        InlineKeyboardButton('✓ Read', callback_data='open:' + str(article_ids[0])),
        InlineKeyboardButton('★ Save', callback_data='save:' + str(article_ids[0]))
    ]]
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    feedback_map = {'like': '👍 Liked', 'dislike': '👎 Disliked', 'open': '✓ Marked read', 'save': '★ Saved'}
    feedback = feedback_map.get(action, action)
    await query.edit_message_reply_markup(reply_markup=None)
    await context.bot.send_message(chat_id=CHAT_ID, text=feedback + ' article #' + str(article_id))
